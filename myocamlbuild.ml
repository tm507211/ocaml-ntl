(* OASIS_START *)
(* DO NOT EDIT (digest: 983030114357c9adbfd159f21b96e959) *)
...
let package_default =
  {
     MyOCamlbuildBase.lib_ocaml = [("ntl", ["src"], [])];
     lib_c = [("ntl", "src", [])];
     flags =
       [
          (["oasis_library_ntl_ccopt"; "compile"],
            [
          ...
          (["oasis_library_ntl_cclib"; "ocamlmklib"; "c"],
            [(OASISExpr.EBool true, S [A "-lntl"; A "-lstdc++"; A "-lgmp"])]);
          (["oasis_executable_test_ccopt"; "compile"],
            [(OASISExpr.EBool true, S [A "-ccopt"; A "-annot"])])
       ];
     includes = [("test", ["src"])]
  }
  ;;
